...
export default function JobList({ jobs, isLoading = false, onJobDeleted }: JobListProps) {
  ...
  // Helper function to get required skills, ensuring it's an array
  const getRequiredSkills = (job: Job) => {
    return Array.isArray(job.required_skills) ? job.required_skills : [];
  };
          ...
          <CardContent>
            ...
            {getRequiredSkills(job).length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {getRequiredSkills(job).slice(0, 3).map((skill) => (
                  <Badge key={skill} variant="secondary" className="text-xs">
                    {skill}
                  </Badge>
                ))}
                {getRequiredSkills(job).length > 3 && (
                  <Badge variant="secondary" className="text-xs">
                    +{getRequiredSkills(job).length - 3} more
                  </Badge>
                )}
              </div>
            )}
