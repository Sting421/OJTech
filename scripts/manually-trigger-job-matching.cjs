// This script manually triggers job matching for a specified job
// Run with: node scripts/manually-trigger-job-matching.cjs <job_id>
...
async function main() {
  try {
    ...
    if (!jobId) {
      ...
      console.error('Usage: node scripts/manually-trigger-job-matching.cjs <job_id>');
      process.exit(1);
    }
    
    console.log(`Manually triggering job matching for job ID: ${jobId}`);
    ...
    console.log(`\nNow users should be able to see this job in their opportunities page.`);
